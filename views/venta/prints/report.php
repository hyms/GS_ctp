            ...
            <?php foreach($item->detalleServicios as $key => $detalle){ ?>
                <tr>
                    ...
                    <?php if($key==0){ ?>
                        ...
                        <td style="border: 0.6px solid black; text-align: right;" rowspan="<?php echo $count;?>"><?php
                                if(!$deuda)
                                    $item->montoPagado = SGServicioVenta::montoPagado($item);

                                echo $item->montoPagado;
                                $totalCobrado=$totalCobrado+$item->montoPagado;
                            ?></td>
                        <td style="border: 0.6px solid black; text-align: right;" rowspan="<?php echo $count;?>"><?php
                                if(($item->montoVenta-$item->montoPagado)>0) {
                                    echo ($item->montoVenta - $item->montoPagado);
                                    $totalsaldo=$totalsaldo+($item->montoVenta-$item->montoPagado);
                                }
                                else
                                {
                                    echo "";
                        ...
                        <td style="border: 0.6px solid black;" rowspan="<?php echo $count;?>"><?php echo $item->fechaVenta; ?></td>
                    <?php } ?>
                </tr>
            <?php }?>
        <?php }?>
